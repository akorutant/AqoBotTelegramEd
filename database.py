...
class DataBase:
    ...
    def add_words(self, chat_id, word):
        words_db = self.cur.execute('SELECT * FROM bad_words WHERE word = ?', (word,)).fetchone()
        default_words = self.cur.execute('SELECT * FROM words WHERE word = ?', (word,)).fetchone()

        if not words_db and not default_words:
    ...
    def get_words_by_chat(self, chat_id, word):
        default_words = self.cur.execute('SELECT word FROM words WHERE word = ?', (word,)).fetchall()
        chat_words = self.cur.execute('SELECT word FROM bad_words WHERE chat_id = ? AND word = ?',
                                      (chat_id, word)).fetchall()
        if default_words or chat_words:
            return True
        return False
    ...
    def add_default_words(self):
        words = self.cur.execute('SELECT * from words').fetchall()
        if not words:
            for word in add_words_list():
                self.cur.execute('INSERT INTO words (word) VALUES (?)', (word.lower(),))
            self.con.commit()
